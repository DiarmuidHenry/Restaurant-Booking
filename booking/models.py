...
class ExceptionalOpeningHours(models.Model):
    ...
    def clean(self):
        ...
        if self.date <= now.date():
            raise ValidationError(
                "Exceptional opening hours must"
                " be for future dates.")
        ...
        if self.is_open:
            if not self.opening_time or not self.closing_time:
                raise ValidationError(
                    "Opening and closing times must"
                    " be set if restaurant is open.")
            if self.opening_time >= self.closing_time:
                raise ValidationError(
                    "Closing time must be later than opening time.")
        else:
...
class RestaurantTable(models.Model):
    ...
    table_location = models.CharField(choices=LOCATION_CHOICES,
                                      default='Inside', max_length=50)
...
class Reservation(models.Model):
    ...
    table_location = models.CharField(
        choices=LOCATION_CHOICES, default='Inside', max_length=50)
    ...
    def reservation_end_time_plus_one_minute(self):
        end_datetime = datetime.combine(
            self.reservation_date, self.reservation_end_time)
