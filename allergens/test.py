...
class MenuItemList(TestCase):
    ...
    def test_allergen_not_ticked_filtering(self):
        ...
        self.assertEqual(response.status_code, 200)
        page_obj = response.context['page_obj']
        # Check that there are items with gluten=False and True in response
        # Check for gluten=True items
        self.assertTrue(any(item.gluten for item in page_obj))
        # Check for gluten=False items
        self.assertTrue(any(not item.gluten for item in page_obj))
    ...
    def test_dietary_requirement_not_ticked_filtering(self):
        # Test that filtering with vegan=false does not affect the response
        response = self.client.get(reverse('menu_items_list'))
        self.assertEqual(response.status_code, 200)
        page_obj = response.context['page_obj']
        # Check that there are items with vegan=False and True in response
        # Check for vegan=True items
        self.assertTrue(any(item.vegan for item in page_obj))
        # Check for vegan=False items
        self.assertTrue(any(not item.vegan for item in page_obj))
